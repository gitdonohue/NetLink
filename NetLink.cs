...
public interface INetLink : IDisposable
{
    ...
    public NetMessage CreateResponse(NetMessage query, bool valid, string response) => new(this, "response", response) { QueryId = query.QueryId, IsQueryResponse = true, IsValid = valid };
}
...
public sealed partial class NetMessage
{
    ...
    public string GetHeader(string key) => Headers.TryGetValue(key, out var header) ? header : string.Empty;
